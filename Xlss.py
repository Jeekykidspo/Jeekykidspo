...
def list_available_tools():
    tools = [
        "1. Nmap - Scan de réseau et détection de services",
        "2. OpenVAS - Scan de vulnérabilités avancé",
        "3. Vérification des en-têtes HTTP",
        "4. Recherche dans les bases de données de fuites (Have I Been Pwned)",
        "5. Analyse des réseaux sociaux",
        "6. Recherche sur Shodan",
        "7. Recherche d'images inversée",
        "8. Scan de vulnérabilités spécifiques d'applications web (OWASP ZAP)",
        "9. Vérification de la sécurité des API",
        "10. Génération de statistiques et graphiques",
        "11. Collecte d’informations sur les sous-domaines",
        "12. Scan de sécurité de la base de données",
        "13. Détection de failles XSS",
        "14. Exploitation de failles de redirection",
        "15. Scan des systèmes d'exploitation",
        "16. Hashing de fichiers",
        "17. Vérification de la sécurité des mots de passe",
        "18. Vérification d'adresses e-mail sur un site",
        "19. Recherche de noms d'utilisateur",
        "20. Vérification des sous-domaines"
    ]
    
    print(Fore.CYAN + "\nOutils disponibles :")
    print(Style.RESET_ALL)  # Réinitialise le style
    for tool in tools:
        print(Fore.GREEN + tool)  # Couleur verte pour les outils
    print(Style.RESET_ALL)  # Réinitialise le style 
...
# Fonction 18 : Vérification d'adresses e-mail sur un site
def check_email_on_site(email, target):
    try:
        response = requests.get(target)
        if email in response.text:
            return f"L'adresse e-mail {email} est présente sur le site {target}."
        else:
            return f"L'adresse e-mail {email} n'est pas présente sur le site {target}."
    except requests.exceptions.RequestException as e:
        return f"Erreur lors de la connexion à {target}: {e}"

# Fonction 19 : Recherche de noms d'utilisateur
def check_username_availability(username, platforms):
    results = {}
    for platform in platforms:
        response = requests.get(f"https://api.example.com/check-username/{platform}/{username}")  # Remplacez par une API réelle
        if response.status_code == 200:
            results[platform] = response.json().get('available', False)
        else:
            results[platform] = "Erreur lors de la vérification."
    return results

# Fonction 20 : Vérification des sous-domaines
def verify_subdomains(domain):
    result = subdomain_lookup(domain)
    if result:
        return f"Sous-domaines trouvés pour {domain} :\n{result}"
    return f"Aucun sous-domaine trouvé pour {domain}."
